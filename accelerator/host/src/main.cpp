...
// Create device buffers
	...
	d_sample = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
				input_size_0*input_size_0*input_channels_0*sizeof(float), sample, &status);
  ...
  d_bn14_out = clCreateBuffer(context, CL_MEM_WRITE_ONLY,
	...
	if (fp == NULL) {
    perror("Error opening convolution weights file");
    } else {
        printf("Convolution weights file opened successfully: %s\n", conv0_wt_file);
        ...
        for (unsigned i = 0; i < output_channels_0; i++) {
            for (unsigned j = 0; j < 32; j++) {
                if (j < kernel_size_0 * kernel_size_0 * input_channels_0 && i < output_channels_0) {
                    if (fscanf(fp, "%f", &wt) != 1) {
                        fprintf(stderr, "Error reading float from convolution weights file at index (%d, %d)\n", i, j);
                        perror("fscanf error");
                        break;  // 退出內部循環，進一步的讀取可能也會失敗
                    }
                    conv0_weight[k++] = wt;
                } else {
                    // 填充以使其成為 CONV_BLOCK_SIZE 的倍數
                    conv0_weight[k++] = 0.0;
                }
            }
        }
    ...
    if (fp == NULL) {
        ...
        // Print a debug message to indicate the file was opened successfully
        printf("File opened successfully for reading: %s\n", bn0_wt_file);

        for(int i = 0; i < output_channels_0; i++) {
            if (fscanf(fp, "%f", &bn0_weight[i]) != 1) {
                // Print an error message if fscanf fails
                fprintf(stderr, "Error reading float from file at index %d\n", i);
                break;  // Exit the loop, as further reads might also fail
            }
        }
...
// Transfer data to the buffers
  ...
  // BATCHNORM 14
    ...
    //status = clWaitForEvents(1, &bn14_event);
    //checkError(status, "Waiting for bn14_event");
    ...
    for (h = 0; h < input_size_0; h++) {
        for (w = 0; w < input_size_0; w++) {
            for (c = 0; c < input_channels_0; c++) {
                image[c * input_size_0 * input_size_0 + h * input_size_0 + w] = sample[k];
                k++;
            }
        }
    }
